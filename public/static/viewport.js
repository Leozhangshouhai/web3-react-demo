...
var adaptUILayout = (function () {
  ...
  var adapt = function (uiWidth) {
    ...
    var meta = meta = document.getElementsByName("viewport")[0];
    meta.content = initialContent;
  };
  return { regulateScreen: regulateScreen, adapt: adapt, viewport: adaptViewport };
})();
var isViewport = adaptUILayout.viewport(size);
if (isViewport) {
  adaptUILayout.adapt(size);
  window.addEventListener("orientationchange", function () { adaptUILayout.adapt(size); })
}
window.onload = function () {
  document.getElementsByTagName("body")[0].classList.add('show');
  try { viewport(adaptUILayout.regulateScreen.cal()) } catch (e) { }
}
